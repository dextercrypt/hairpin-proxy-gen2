...
echo -e "  ${DIM}  Controller  :${RESET}  ${WHITE}dextercrypt/hairpin-proxy-gen2-controller:v1.0.5${RESET}"

if [[ "$MODE" == "gateway" || "$MODE" == "both" ]]; then
  echo -e "  ${DIM}  HAProxy (Gateway API) :${RESET}  ${WHITE}dextercrypt/hairpin-proxy-gen2-haproxy:v1.0.5${RESET}"
  echo -e "  ${DIM}  Gateway target        :${RESET}  ${CYAN}${GATEWAY_TARGET}${RESET}"
fi
if [[ "$MODE" == "ingress" || "$MODE" == "both" ]]; then
  echo -e "  ${DIM}  HAProxy (Ingress)     :${RESET}  ${WHITE}dextercrypt/hairpin-proxy-gen2-haproxy:v1.0.5${RESET}"
